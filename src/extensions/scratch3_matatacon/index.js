    ...
    parseCommand() {
        ...
        switch(command_data[1]) {
            case BLECommand.CMD_SENSOR_DETECT: {
                console.log("receive sensor detect rsp(%d)!", command_data[2]);
                if(command_data[2] == SensorDetectCommand.MOTION_STATUS_DETECT) {
                    this.commandSyncFlag.motionSensorStatusFlag = false;
                    this.sensor_status = command_data[3];
                }
                break;
            }
...
class Scratch3MatataConBlocks {
    ...
    motionSensorStatus (args) {
        const motionSensorStatusData = new Array();
        let detect_status = 1;
        if (args.SENSOR_STATUmotionSensorStatusS == MotinStatusMenu.SHAKEN) {
            detect_status = 1;
        ...
        motionSensorStatusData.push(BLECommand.CMD_SENSOR_DETECT);
        motionSensorStatusData.push(SensorDetectCommand.MOTION_STATUS_DETECT);
        motionSensorStatusData.push(detect_status);

        this._peripheral.commandSyncFlag.motionSensorStatusFlag = true;
        this._peripheral.send(this._peripheral.packCommand(motionSensorStatusData));
        return new Promise(resolve => {
            let count = 0;
            let interval = setInterval(()=> {
                if(count > 1500) {
                    console.log("motionSensorStatus timeout!");
                    clearInterval(interval);
                    this._peripheral.commandSyncFlag.motionSensorStatusFlag = false;
                    resolve(0);
                }
                else if(this._peripheral.commandSyncFlag.motionSensorStatusFlag == false) {
                    clearInterval(interval);
                    resolve(this._peripheral.sensor_status);
                }
                count += 10;
            }, 10);
        });
